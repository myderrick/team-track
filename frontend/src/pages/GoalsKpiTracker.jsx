...
// ─────────────────────────────────────────────────────────────────────────────
const cardCls =
  "rounded-2xl border p-6 shadow-sm bg-[var(--card)] text-[var(--fg)] border-[var(--border)]";
const softCardCls =
  "rounded-xl border p-6 shadow-sm bg-[var(--card)] text-[var(--fg)] border-[var(--border)]";
const pillMuted =
  "text-xs px-2 py-1 rounded-full bg-[var(--surface)] text-[var(--fg-muted)]";
...
function statusColor(p) {
  if (p >= 90)
    return "bg-[color-mix(in oklab,green 20%,transparent)] text-green-600 dark:text-green-400";
  if (p >= 60)
    return "bg-[color-mix(in oklab,orange 20%,transparent)] text-amber-600 dark:text-amber-400";
...
const fmtMeasure = (n, unit, currency_code) => {
  if (currency_code)
    return `${currencySymbol(currency_code)} ${Number(
      n || 0
    ).toLocaleString()}`;
...
function groupByOrgGoal(goals = [], rows = [], mode = "avg") {
  ...
  for (const [key, rs] of byGroup.entries()) {
    ...
    const groupedByGoal = [...goalIds].map((id) => ({
      id,
      rows: rs.filter((x) => x.goal_id === id),
    }));
    const current = groupedByGoal.reduce(
      (sum, g) => sum + computeCurrent(g.rows, mode),
      0
    );
    const target = [...goalIds].reduce(
      (sum, id) => sum + Number(byId.get(id)?.target || 0),
      0
    );
    const pct =
      target > 0 ? Math.round(Math.min(100, (current / target) * 100)) : 0;
...
function MiniGoalCard({goal, currentValue, fmt}) {
  ...
  const p =
    t > 0
      ? Math.max(
          0,
          Math.min(100, Math.round((Number(currentValue || 0) / t) * 100))
        )
      : 0;
  ...
  return (
    <div className="snap-start shrink-0 min-w-[260px] md:min-w-[320px] lg:min-w-[360px] rounded-xl border p-4 shadow-sm bg-[var(--card)] text-[var(--fg)] border-[var(--border)]">
      ...
      <div
        className={`mt-2 text-[11px] flex items-center justify-between ${textMuted}`}
      >
        <div>
          Current:{" "}
          <strong className="text-[var(--fg)]">
            {fmt(currentValue, goal.unit, null)}
          </strong>
        </div>
        <div>
          Target:{" "}
          <strong className="text-[var(--fg)]">
            {fmt(goal.target, goal.unit, null)}
          </strong>
        </div>
      </div>
    </div>
...
function CategoryRail({title, blocks, fmt}) {
  ...
  const sorted = blocks.slice().sort((a, b) => {
    const tA = Number(a.goal.target || 0),
      tB = Number(b.goal.target || 0);
            ...
            return (
              <li key={idx} className="flex justify-between gap-4">
                ...
                <span className="font-medium">
                  {fmt(pt.value, goal.unit, goal.currency_code)}
                </span>
              </li>
            );
          })}
  ...
  return (
    <div className={softCardCls}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${statusColor(
            pct
          )}`}
        >
          {pct}% to target
      ...
      <div className="h-16">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart
            data={series.length ? series : [{value: currentValue, i: 0}]}
          >
            <Line
              type="monotone"
              dataKey="value"
      ...
      <div>
        ...
        >
          <div>
            Current:{" "}
            <strong className="text-[var(--fg)]">
              {fmt(currentValue, goal.unit, goal.currency_code)}
            </strong>
          </div>
          <div>
            Target:{" "}
            <strong className="text-[var(--fg)]">
              {fmt(goal.target, goal.unit, goal.currency_code)}
            </strong>
          </div>
        </div>
      </div>
...
function normalizeGoal(g) {
  ...
  const normalizedAlignment =
    (g.org_goal_label ??
      meta.alignment_label ??
      (g.org_goal_id ? "Org Goal" : ORG_GOAL_UNALIGNED)) ||
    ORG_GOAL_UNALIGNED;
  ...
  return {
    ...
    currency_code:
      meta.measure_currency ?? g.currency_code ?? g.currency ?? null,
    unit,
    target: g.target ?? g.target_value ?? g.target_amount ?? null,
    start_value: g.start_value ?? null,
    category:
      (g.category ?? g.goal_category ?? meta.category ?? null)?.toLowerCase() ||
      null,
...
export default function GoalsKpiTracker() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let mergedGoals = baseGoals.map(normalizeGoal).filter((g) => {
      ...
      if (selectedEmployeeId) {
        const eg = await supabase
          .schema("public")
          .rpc("employee_dashboard", {p_employee_id: selectedEmployeeId});
        if (!eg.error) {
          const selfGoals = (eg.data?.goals || [])
            .map(normalizeGoal)
            .filter(
              (g) => g.self_selected === true || g.visibility === "personal"
            );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [mRes, sRes] = await Promise.all([
        ...
        supabase
          .schema("public")
          .rpc("org_user_completion_summary", {
            p_org_id: orgId,
            p_start: startStr,
            p_end: endStr,
          }),
  ...
  const nameOptions = useMemo(() => {
    const names = employees
      .filter(
        (e) => department === "All Departments" || e.department === department
      )
      .map((e) => e.full_name)
      .filter(Boolean);
  ...
  const goalTypeOptions = useMemo(
    () => ["All Goals", ...Array.from(new Set(goals.map((g) => g.title)))],
    [goals]
  );
  ...
  const orgGoalOptions = useMemo(() => {
    ...
    for (const g of goals) {
      const raw =
        g.alignment_label ?? (g.org_goal_id ? "Org Goal" : ORG_GOAL_UNALIGNED);
  ...
  const filteredRows = useMemo(() => {
    return (measurements || []).filter((r) => {
      if (department !== "All Departments" && r.department !== department)
        return false;
      ...
      if (orgGoal !== "All Org Goals") {
        ...
        const label =
          g?.alignment_label || (g?.org_goal_id ? "Org Goal" : "Unaligned");
  ...
  const numericRegularBlocks = useMemo(() => {
    const typeGoals = goals.filter(
      (g) =>
        ...
        !CATEGORY_KEYS.includes((g.category || "").toLowerCase())
    );
    return typeGoals
      .map((goal) => {
        const rows = filteredRows.filter((r) => r.goal_id === goal.id);
  ...
  const categoryBlocks = useMemo(() => {
    ...
    for (const key of CATEGORY_KEYS) {
      ...
      if (selectedEmployeeId)
        catGoals = catGoals.filter(
          (g) => g.owner_employee_id === selectedEmployeeId
        );
  ...
  const monetaryAgg = useMemo(() => {
    ...
    const totalTarget = withTargets.reduce(
      (s, b) => s + Number(b.goal.target || 0),
      0
    );
    const totalCurrent = withTargets.reduce(
      (s, b) => s + Number(b.current || 0),
      0
    );
    ...
    monetaryAgg.totalTarget > 0
      ? Math.max(
          ...
          Math.min(
            100,
            Math.round(
              (monetaryAgg.totalCurrent / monetaryAgg.totalTarget) * 100
            )
          )
        )
      : 0;
  ...
  const bottomPerformers = useMemo(() => {
    ...
    return s
      .filter(
        (p) => !topPerformers.find((t) => t.employee_id === p.employee_id)
      )
      .slice(0, 3);
  ...
  const visibleSections = useMemo(() => {
    const sections = [];
    if (activeMeasureTypes.includes("monetary") && monetaryBlocks.length > 0)
      sections.push("monetary");
    if (numericRegularBlocks.length > 0) sections.push("numeric");
    for (const key of CATEGORY_KEYS)
      if (categoryBlocks[key]?.length) sections.push(`cat:${key}`);
    if (
      activeMeasureTypes.includes("qualitative") &&
      qualitativeList.length > 0
    )
      sections.push("qualitative");
    return sections;
  }, [
    activeMeasureTypes,
    monetaryBlocks,
    numericRegularBlocks,
    qualitativeList,
    categoryBlocks,
  ]);
  ...
  const atRisk = useMemo(() => {
    ...
    return [...monetaryBlocks, ...numericRegularBlocks]
      .map((b) => {
        const d = goalsById.get(b.goal.id)?.deadline
          ? new Date(goalsById.get(b.goal.id).deadline)
          : null;
  ...
  return (
    <div className="flex flex-col h-screen bg-[var(--app-bg)] text-[var(--fg)]">
      ...
      <GoalsFilterBar
        ...
        subtitle={
          quarter && quarter !== "All"
            ? `Performance overview for ${quarter}`
        ...
        setDepartment={(v) => {
          ...
          if (v !== "All Departments") {
            const valid = employees.some(
              (e) => e.full_name === teamFilter && e.department === v
            );
            ...
            <div className={`grid ${gridClass} gap-6 mb-10`}>
              ...
              {visibleSections.includes("numeric") && (
                <section className={softCardCls}>
                  ...
                  {numericRegularBlocks.length === 0 ? (
                    <EmptyState
                      title="No numeric goals"
                      subtitle="Create numeric goals to track counts/quantities."
                    />
                  ) : (
                    <div className="grid grid-cols-1 gap-4">
                      {numericRegularBlocks.map(
                        ({goal, current, series, last3}) => (
                          <GoalStatCard
                            key={goal.id}
                            goal={{...goal, currency_code: null}}
                            currentValue={current}
              ...
              {visibleSections.includes("qualitative") && (
                <section className={softCardCls}>
                  ...
                  {qualitativeList.length === 0 ? (
                    <EmptyState
                      title="No qualitative goals"
                      subtitle="Create qualitative goals to track status."
                    />
                  ) : (
                    <div className="grid grid-cols-1 gap-4">
                      {qualitativeList.map((goal) => {
                        ...
                        return (
                          <div key={goal.id} className={softCardCls}>
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0 w-full">
                            ...
                            <p className={`text-sm ${textMuted} mt-2`}>
                              Track notes, milestones, or review outcomes. You
                              can attach checklists or rubrics to quantify
                              progress.
                            </p>

                            <div
                              className={`flex items-center gap-2 text-xs ${textMuted} mt-1`}
                            >
                    ...
                    {atRisk.slice(0, 6).map((x) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-semibold shrink-0 ${statusColor(
                              x.pct
                            )}`}
                          >
                            {x.pct}%
            ...
            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={softCardCls}>
                ...
                {topPerformers.length === 0 ? (
                  <div className={`text-sm ${textMuted}`}>No data</div>
                ) : (
                  <ul className="space-y-1">
                    {topPerformers.map((p) => (
